...
class TmxTileSet
{
	...
	public function new(data:Dynamic)
	{
		...
		else // internal
		{
			...
			if (node.has.trans) trans = Std.parseInt("0x" + node.att.trans);
			...
			setImage(Assets.getBitmapData(imageSource.split("..").join("data")));
		}
	}
	...
	public function setImage(v:BitmapData):BitmapData
	{
		...
		numCols = Math.floor((v.width-2*margin) / (tileWidth+spacing)) ;
		numRows = Math.floor((v.height-2*margin) / (tileHeight+spacing));
	...
	public function getRect(id:Int):Rectangle
	{
		...
		return new Rectangle
			(
			margin + Math.floor(id%numCols)*(tileWidth + spacing),
			margin + Math.floor(id/numCols)*(tileHeight + spacing),
			tileWidth,
			tileHeight
			);
	}
